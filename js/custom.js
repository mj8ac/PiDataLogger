...
function charts() {
	...
	function initChartData3() {
		for (var i=0; i<totalPoints; ++i)
			points3.push(0);
	}
	
	function initChartData4() {
		for (var i=0; i<totalPoints; ++i)
			points4.push(0);
	}
	
	function initT1ChartData() {
		for (var i=0; i<t1Points; ++i)
			t1Points.push(0);
	}
	
	function initT2ChartData() {
		for (var i=0; i<t2Points; ++i)
			t1Points.push(0);
	}
	
	function initT3ChartData() {
		for (var i=0; i<t3Points; ++i)
			t1Points.push(0);
	}
	...
	function getTemperatureData(){
		...
		$.get("/t1realTimeData.txt", function(data){
			temperature = parseFloat(data);
			$("#chartTitle").text("T1: " + temperature);
	...
	function getTemperatureData2(){
		...
		$.get("/t2realTimeData.txt", function(data){
			temperature = parseFloat(data);
			$("#chart2Title").text("T2: " + temperature);
			
			if (points2.length > 0) {
				points2 = points2.slice(1);
			}
		
			while (points2.length < totalPoints){
				points2.push(temperature);
			}
				
			},"text");
		
		for (var i=0; i<points2.length; ++i) {
			res2.push([i, points2[i]]);
		}
	...
	function getTemperatureData3(){
		...
		$.get("/t3realTimeData.txt", function(data){
			temperature = parseFloat(data);
			$("#chart3Title").text("T3: " + temperature);
			
			if (points3.length > 0) {
				points3 = points3.slice(1);
			}
		
			while (points3.length < totalPoints){
				points3.push(temperature);
			}
				
			},"text");
		
		for (var i=0; i<points3.length; ++i) {
			res3.push([i, points3[i]]);
		}
	...
	function getTemperatureData4(){
		...
		$.get("/temperaturedata2.txt", function(data){
			temperature = parseFloat(data);
			$("#chart3Title").text("Wall: " + temperature);
	...
	{	
		var options = {
			...
			yaxis: { min: 0, max: 35, tickFormatter: function (v) { return v + "%"; }},
			xaxis: { show: false },
			colors: ["#FA5833"],
	...
	if($("#realtimechart").length)
	{
		...
		initChartData();
		var plot = $.plot($("#realtimechart"), [ getTemperatureData() ], options);
		function update() {
			plot.setData([ getTemperatureData() ]);
			plot.setupGrid();
			// since the axes don't change, we don't need to call plot.setupGrid()
			plot.draw();
	...
	if($("#t1StaticChart").length)
	{
		var ambOptions = {
			series: { shadowSize: 1 },
			lines: { fill: true, fillColor: { colors: [ { opacity: 1 }, { opacity: 0.1 } ] }},
			//yaxis: { min: 0, max:  100},
			xaxis: { show: true, mode: "time", timezone: "%H%M" },
			colors: ["#F4A506"],
			//points: { show:false },
			grid: {	tickColor: "#dddddd",
					borderWidth: 0 
		...
		function updateStaticAmbChart() {
			$.get("t1LogChartData.json", function(t1Data){
				$.plot("#t1StaticChart", [t1Data], ambOptions);
			}, "json");
			$.get("t2LogChartData.json", function(t2data){
				$.plot("#t2StaticChart", [t2data], ambOptions);
			}, "json");
			$.get("t3LogChartData.json", function(t3data){
				$.plot("#t3StaticChart", [t3data], ambOptions);
			}, "json");
			setTimeout(updateStaticAmbChart, updateInterval);			
		}
		
		updateStaticAmbChart();
	}
	...
	if($("#realtimechart2").length)
	{
		...
		initChartData2();
		var plot2 = $.plot($("#realtimechart2"), [ getTemperatureData2() ], options);
		function update2() {
			plot2.setData([ getTemperatureData2()]);
			// since the axes don't change, we don't need to call plot.setupGrid()
			plot2.setupGrid();
			plot2.draw();
	...
	if($("#realtimechart3").length)
	{
		...
		initChartData3();
		var plot3 = $.plot($("#realtimechart3"), [ getTemperatureData3() ], options);
		function update3() {
			plot3.setData([ getTemperatureData3() ]);
			// since the axes don't change, we don't need to call plot.setupGrid()
			plot3.setupGrid();
			plot3.draw();
